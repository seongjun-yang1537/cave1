...
namespace Outgame
{
    [CustomEditor(typeof(ShopScheduleTable))]
    public class EditorShopScheduleTable : Editor
    {
        ShopScheduleTable script;
        IntRange newDay = new IntRange(0, 0);
        ShopScheduleData newSchedule;
        void OnEnable()
        {
            script = (ShopScheduleTable)target;
            if (script.table == null) script.table = new List<ShopScheduleTable.Entry>();
        }
        ...
        SUIElement RenderInput()
        {
            ...
            .Content(
                ...
                + SEditorGUILayout.Var("", newSchedule).OnValueChanged(v => newSchedule = (ShopScheduleData)v)
                + SEditorGUILayout.Button("+").Width(16).OnClick(() =>
                {
                    ...
                    if (entry == null)
                    {
                        entry = new ShopScheduleTable.Entry { days = new IntRange(newDay), schedules = new List<ShopScheduleData>() };
        ...
        SUIElement RenderScheduleList(ShopScheduleTable.Entry entry)
        {
            ...
            for (int i = 0; i < entry.schedules.Count; i++)
            {
                ...
                .Content(
                    SEditorGUILayout.Var("", sched).OnValueChanged(v => entry.schedules[idx] = (ShopScheduleData)v)
