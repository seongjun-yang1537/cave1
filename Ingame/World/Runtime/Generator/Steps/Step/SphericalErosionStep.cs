...
namespace World
{
    public class SphericalErosionStep : IWorldgenRasterizeStep
    {
        private readonly int radius;
        private readonly int iterations;

        public SphericalErosionStep(int radius = 1, int iterations = 1)
        {
        ...
        private struct SphericalErosionJob : IJobParallelFor
        {
            ...
            public void Execute(int x)
            {
                ...
                for (int y = 0; y < outputSize.y; y++)
                {
                    ...
                    for (int z = 0; z < outputSize.z; z++)
                    {
                        int baseZ = z + radius;
                        int val = 0;
                        for (int i = -radius; i <= radius && val == 0; i++)
                            for (int j = -radius; j <= radius && val == 0; j++)
                                for (int k = -radius; k <= radius && val == 0; k++)
                                {
                                    if (i * i + j * j + k * k > rSq)
                                        continue;
                                    int nx = baseX + i;
        ...
        private static void Erode(ChunkedScalarField field, int radius)
        {
            ...
            foreach (var coord in chunkCoords)
            {
                ...
                var job = new SphericalErosionJob
                {
                    input = input,
                    output = output,
