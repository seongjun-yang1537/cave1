...
namespace World
{
    public class CubicErosionStep : IWorldgenRasterizeStep
    {
        private readonly int iterations;
        private readonly int kernelSize;

        public CubicErosionStep(int iterations = 1, int kernelSize = 5)
        {
            ...
            public void Execute(int x)
            {
                ...
                for (int y = 0; y < outputSize.y; y++)
                {
                    ...
                    for (int z = 0; z < outputSize.z; z++)
                    {
                        int baseZ = z + half;
                        int val = 1;
                        for (int i = -half; i <= half && val == 1; i++)
                            for (int j = -half; j <= half && val == 1; j++)
                                for (int k = -half; k <= half && val == 1; k++)
                                {
                                    int nx = baseX + i;
                                    int ny = baseY + j;
                                    int nz = baseZ + k;
        ...
        private static void Erode(ChunkedScalarField field, int kernelSize)
        {
